...
void Fbx::Load(LPCSTR fileName)
{
	...
	if (FAILED(D3DXCreateEffectFromResource(g.pDevice, NULL, MAKEINTRESOURCE(IDR_SHADOW_MAP), NULL, NULL, NULL, NULL, &_shaderShadowMap, &error)))
	{
...
void Fbx::Draw()
{
	...
	// プロジェクション行列をセット
	_shader->SetMatrix("matProj", &Projection());

	// 回転を反映してライトを一定の向きから当たっているようにする(正しい影がつくようにする)
	// ワールド行列で行うので『 Z * X * Y』の順で掛ける
	// 掛ける順番が違うと正しく影がつかない
	_shader->SetMatrix("matRotate", &(Rotation().rotateZ * Rotation().rotateX * Rotation().rotateY));
...
D3DXMATRIX Fbx::World()
{
	//ワールド行列
	//Unityを参考にした回転の順番
	D3DXMATRIX world;
	world = Scaling() * Rotation().rotateZ * Rotation().rotateX * Rotation().rotateY * Translation();
	g.pDevice->SetTransform(D3DTS_WORLD, &world);
	return world;
}

D3DXMATRIX Fbx::Scaling()
{
	//拡大縮小
	D3DXMATRIX scale;
...
D3DXMATRIXROTATE Fbx::Rotation()
{
	//回転行列
	D3DXMATRIXROTATE rotation;
	D3DXMatrixRotationX(&rotation.rotateX, D3DXToRadian(_rotate.x));
	D3DXMatrixRotationY(&rotation.rotateY, D3DXToRadian(_rotate.y));
	D3DXMatrixRotationZ(&rotation.rotateZ, D3DXToRadian(_rotate.z));

	return rotation;
}
D3DXMATRIX Fbx::Translation()
{
	//移動行列
	D3DXMATRIX trans;
...
// Shadowクラスに移動
//void Fbx::DrawShadowMap(LPDIRECT3DTEXTURE9 renderTexture)
//{
//	//頂点バッファをセット
//	g.pDevice->SetStreamSource(0, _vertexBuffer, 0, sizeof(Vertex));
//
...
//	_shaderShadowMap->SetMatrix("matView", &lightView);
//
//	// プロジェクション行列
//	D3DXMATRIX proj;
//	g.pDevice->GetTransform(D3DTS_PROJECTION, &proj);
//	_shaderShadowMap->SetMatrix("matProj", &proj);
//
//
//	// シャドウマップ
//	_shader->SetMatrix("matUV", &_shadow->ShadowUV());
//	_shader->SetMatrix("matLightView", &lightView);
...
void Fbx::SetShader(LPD3DXEFFECT setShader)
{
	_shader = setShader;
}
void Fbx::SetShaderPass(int pass)
{
	for (DWORD i = 0; i < materialCount; i++)
	{
		_materialData[i].shaderPass = pass;
	}
}